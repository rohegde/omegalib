#ifndef __EIGENWRAP__LINEAR_LEAST_SQUARES__HPP__
#define __EIGENWRAP__LINEAR_LEAST_SQUARES__HPP__

#include <eigenwrap/vector3.h>
...
eigenwrap::matrix_mxn< number_of_unknowns, 1, float_t >
solve( std::vector< Vector3< float_t > >& data_points, float_t tolerance = 1e-9 )
{
    ...
    eigenwrap::matrix_mxn< number_of_unknowns, number_of_data_points > Xt;
    X.transposeTo( Xt );
    
    std::cout << Xt << std::endl;
    
    eigenwrap::matrix_mxm< number_of_unknowns > XtX;
    XtX.mul( Xt, X );
    
    eigenwrap::matrix_mxm< number_of_unknowns > XtX_inverse;
	XtX.computeInverse( XtX_inverse, tolerance );

    eigenwrap::matrix_mxn< number_of_unknowns, number_of_data_points > XtX_inv_mul_Xt;
    
    XtX_inv_mul_Xt.mul( XtX_inverse, Xt );
    
    eigenwrap::matrix_mxn< number_of_unknowns, 1 > beta_hat;
