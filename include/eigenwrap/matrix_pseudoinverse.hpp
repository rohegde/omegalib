#ifndef __EIGENWRAP__MATRIX_PSEUDOINVERSE__HPP__
#define __EIGENWRAP__MATRIX_PSEUDOINVERSE__HPP__
...
#include <cstddef>
#include <functional>
#include <eigenwrap/lapack_svd.hpp>
...
namespace eigenwrap
{
	...
	public:		
						...
						typename T::value_type tolerance = std::numeric_limits< typename T::value_type >::epsilon() )
		{
			...
			if ( T::ROWS < T::COLS ) {
				EIGENWRAP_ERROR( "matrix compute_pseudoinverse - number of matrix rows have to be greater or equal to number of matrix columns.", EIGENWRAP_HERE );
			}
			...
			if ( !ok ) {
				EIGENWRAP_ERROR( "matrix compute_pseudoinverse - problem with lapack svd.", EIGENWRAP_HERE );
			}
